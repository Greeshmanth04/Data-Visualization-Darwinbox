...
import { Dashboard, DashboardWidget, Dataset, AIAnalysisResult, User } from '../types';
...
const WidgetRenderer: React.FC<{ widget: DashboardWidget; data: any[] }> = ({ widget, data }) => {
  ...
  console.log('[DEBUG] WidgetRenderer:', {
    id: widget.id,
    type: widget.type,
...
}> = ({ datasets, initialWidget, onClose, onSave }) => {
                ...
                {['bar', 'line', 'pie', 'metric'].includes(type) && (
                  <div>
                    ...
                    >
                      ...
                      {selectedDataset.columns.filter(c => c.type === 'number').map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
                    </select>
                  </div>
                )}
              </>
                ...
                {currentDashboard.widgets.map((widget, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!dataset ? (
                        <div className="flex items-center justify-center h-full text-red-400">Dataset missing</div>
                      ) : (
                        <div className={`p-5 h-full flex flex-col ${isEditMode ? 'opacity-60 blur-[1px]' : ''}`}>
                          ...
                          <div className="flex-1 min-h-0 w-full">
                            <WidgetRenderer widget={widget} data={dataset.data} />
                          </div>
                        </div>
                      )}
                    </div>
