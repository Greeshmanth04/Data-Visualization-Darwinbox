...
import { ViewState, Dataset, Dashboard, User, UserRole } from './types';
...
const App: React.FC = () => {
  ...
  // Initialize Admin seed (Only needed for Mock mode, but harmless to call)
  useEffect(() => {
    // In real app, DB seeding happens on server start.
    // For mock mode fallback:
    const storedUsers = JSON.parse(localStorage.getItem('lumina_users') || '[]');

    // Seed standard mock users if none exist or are missing
    const usersToSeed = [
      {
        id: 'admin_01',
        name: 'System Admin',
      ...
      ...MOCK_USERS.map(u => ({ ...u, password: 'password123' })) // Add default password for mock users
    ];

    let updated = [...storedUsers];
    let changed = false;

    usersToSeed.forEach(user => {
      if (!storedUsers.some((u: any) => u.email.toLowerCase() === user.email.toLowerCase())) {
        updated.push(user);
        changed = true;
      }
    });
  ...
  useEffect(() => {
    if (user) {
      ...
      api.dashboards.getAll(user.id).then(data => {
        console.log(`[DEBUG] Dashboards received for user ${user.id}:`, data);
        setDashboards(data);
      });
    }
  }, [user]);
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'catalog':
        return <DataCatalog datasets={datasets} currentUser={user} onUpdateDataset={handleUpdateDataset} />;
      case 'knowledge':
