...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'catalog':
        return <DataCatalog datasets={datasets} currentUser={user} onUpdateDataset={handleUpdateDataset} onRefreshDatasets={fetchData} onAddDataset={(ds) => setDatasets(prev => [...prev, ds])} />;
      case 'schema':
        return <SchemaView datasets={datasets} onUpdateDataset={handleUpdateDataset} onAddDataset={(ds) => setDatasets(prev => [...prev, ds])} currentUser={user} />;
      case 'knowledge':
