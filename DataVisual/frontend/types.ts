...
export type RowPolicyOperator = 'eq' | 'neq' | 'contains' | 'gt' | 'lt' | 'gte' | 'lte';

export interface RowPolicy {
  role: UserRole;
  column: string;
  operator: RowPolicyOperator;
...
export interface Dataset {
  ...
  rowPolicies?: RowPolicy[]; // Row-level access control policies (admin only)
...
export type JoinType = '1:1' | '1:N' | 'N:1';

export interface CrossDBRelationship {
  id: string;
