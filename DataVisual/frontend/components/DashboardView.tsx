...
const WidgetBuilderModal: React.FC<{
  ...
  // Source type toggle: 'mongo' | 'cache'
  const [sourceTab, setSourceTab] = React.useState<'mongo' | 'cache'>('mongo');

  // Cache state
  const [cacheEntries, setCacheEntries] = React.useState<any[]>([]);
  const [cacheLoading, setCacheLoading] = React.useState(false);
  const [selectedCacheKey, setSelectedCacheKey] = React.useState('');
  const [cacheRows, setCacheRows] = React.useState<any[]>([]);
  const [cacheColumns, setCacheColumns] = React.useState<Array<{ name: string; type: string }>>([]);

  // Load cache list when tab switches to cache
  React.useEffect(() => {
    if (sourceTab !== 'cache') return;
    setCacheLoading(true);
    api.cache.list()
      .then(entries => { setCacheEntries(entries); if (entries.length > 0 && !selectedCacheKey) setSelectedCacheKey(entries[0].key); })
      .catch(() => { })
      .finally(() => setCacheLoading(false));
  }, [sourceTab]);

  // Load cache rows when selected key changes
  React.useEffect(() => {
    if (!selectedCacheKey) return;
    api.cache.getData(selectedCacheKey)
      .then(payload => { setCacheRows(payload.rows || []); setCacheColumns(payload.columns || []); })
      .catch(() => { setCacheRows([]); setCacheColumns([]); });
  }, [selectedCacheKey]);
  ...
  const visibleColumns = useMemo(() => {
    if (sourceTab === 'cache') return cacheColumns;
    if (!selectedDataset) return [];
    return selectedDataset.columns.filter(c => !restrictedCols.includes(c.name));
  }, [sourceTab, selectedDataset, restrictedCols, cacheColumns]);

  // Reset axes when source changes
  React.useEffect(() => { setXAxis(''); setDataKey(''); }, [sourceTab, selectedCacheKey, datasetId]);

  const handleSave = () => {
    const isCacheSource = sourceTab === 'cache';
    const effectiveDatasetId = isCacheSource ? selectedCacheKey : datasetId;
  ...
  // Preview data — mongo rows or cached rows
  const previewData: any[] = sourceTab === 'cache' ? cacheRows : (selectedDataset?.data || []);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden h-[85vh]">
        ...
        <div className="w-full md:w-1/3 bg-slate-950 border-r border-slate-800 p-6 flex flex-col overflow-y-auto">
          <h3 className="text-xl font-bold text-white mb-4">{initialWidget ? 'Edit Widget' : 'Add Widget'}</h3>

          <div className="space-y-6">

            {/* Source type tab toggle */}
            <div>
              <label className="block text-xs font-medium text-slate-500 uppercase mb-2">Data Source</label>
              <div className="flex rounded-lg overflow-hidden border border-slate-700">
                <button
                  onClick={() => setSourceTab('mongo')}
                  className={`flex-1 py-1.5 text-xs font-medium transition-colors flex items-center justify-center gap-1 ${sourceTab === 'mongo' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                    }`}
                >
                  <DataIcon size={12} /> MongoDB Datasets
                </button>
                <button
                  onClick={() => setSourceTab('cache')}
                  className={`flex-1 py-1.5 text-xs font-medium transition-colors flex items-center justify-center gap-1 ${sourceTab === 'cache' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200'
                    }`}
                >
                  ⚡ Cached Datasets
                </button>
            ...
            {/* Dataset / Cache selector */}
            <div>
              <label className="block text-xs font-medium text-slate-500 uppercase mb-2">
                {sourceTab === 'cache' ? 'Cached Dataset' : 'Dataset'}
              </label>
              {sourceTab === 'cache' ? (
                cacheLoading ? (
                  <div className="text-slate-500 text-xs py-2">Loading cached datasets…</div>
                ) : cacheEntries.length === 0 ? (
                  <div className="text-slate-600 text-xs py-2">No cached datasets. Cache data first in Data Catalog.</div>
                ) : (
                  <select
                    value={selectedCacheKey}
                    onChange={e => setSelectedCacheKey(e.target.value)}
                    className="w-full bg-slate-800 border border-slate-700 rounded p-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  >
                    {cacheEntries.map(e => (
                      <option key={e.key} value={e.key}>{e.sourceName} ({e.rowCount.toLocaleString()} rows · {e.sourceType})</option>
                    ))}
                  </select>
                )
              ) : (
            ...
            {/* Axis Selection — shown when columns are available */}
                ...
                {['bar', 'line', 'pie'].includes(type) && (
                  <div>
                    ...
                    >
                      ...
                      {visibleColumns.map((c: any) => <option key={c.name} value={c.name}>{c.displayName || c.description || c.name}</option>)}
                    </select>
                  </div>
                )}
            ...
            {/* Size */}
            <div>
              <label className="block text-xs font-medium text-slate-500 uppercase mb-2">Size</label>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">Width (1-12)</label>
                  <input type="number" min="1" max="12" value={width}
                ...
                <div>
                  ...
                  <input type="number" min="1" max="4" value={height}
          ...
          <div className="mt-auto pt-6 flex space-x-3">
            <button onClick={onClose} className="flex-1 py-2 rounded border border-slate-700 text-slate-400 hover:text-white hover:bg-slate-800">Cancel</button>
        ...
        <div className="flex-1 bg-slate-900 p-8 flex flex-col overflow-y-auto">
          ...
          <div className="flex-1 flex items-center justify-center bg-slate-950/50 rounded-xl border border-slate-800 border-dashed p-4 min-h-[400px]">
            {previewData.length > 0 && (xAxis || type === 'metric') && dataKey ? (
              <div className="w-full" style={{ maxWidth: `${(width / 12) * 100}%` }}>
                <div className={`bg-slate-800 border border-slate-700/50 rounded-xl p-5 shadow-sm flex flex-col transition-all duration-300 ${height === 1 ? 'h-32' : height === 2 ? 'h-64' : height === 3 ? 'h-96' : 'h-[32rem]'
                  ...
                  <div className="flex-1 min-h-0">
                    <WidgetRenderer
                      widget={{ id: 'preview', title, type, datasetId: sourceTab === 'cache' ? selectedCacheKey : datasetId, config: { xAxis, dataKey, color, aggregation }, w: width, h: height }}
                      data={previewData}
              ...
              <div className="text-slate-600 flex flex-col items-center">
                ...
                {sourceTab === 'cache' && !selectedCacheKey && <p className="text-xs mt-2">No cached datasets available</p>}
              </div>
            )}
          </div>
        </div>
...
const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  React.useEffect(() => {
    if (!currentDashboard) return;
    const cacheKeys: string[] = Array.from(new Set(
      currentDashboard.widgets
        .map(w => w.datasetId)
        .filter((id): id is string => typeof id === 'string' && id.startsWith('data:'))
    ));
    cacheKeys.forEach(key => {
      if (cacheDataMap[key]) return; // already fetched
      api.cache.getData(key)
        .then(payload => {
          setCacheDataMap(prev => ({ ...prev, [key as string]: { rows: payload.rows || [], columns: payload.columns || [] } }));
        })
        .catch(() => {
          setCacheDataMap(prev => ({ ...prev, [key as string]: { rows: [], columns: [] } }));
        });
    });
  }, [currentDashboard]);
            ...
            {currentDashboard.widgets.length > 0 ? (
              <div className="grid grid-cols-12 gap-6">
                {currentDashboard.widgets.map((widget, index) => {
                  const isCacheWidget = !!widget.datasetId?.startsWith('data:');
                  const cacheEntry = isCacheWidget ? cacheDataMap[widget.datasetId] : null;
                  const dataset = isCacheWidget ? null : datasets.find(d => d.id === widget.datasetId);

                  // Access control (only applies to MongoDB datasets)
                  ...
                  const isDatasetRestricted = !isCacheWidget && !!widgetPolicy && !widgetPolicy.canView;
                  const usesRestrictedCol = !isCacheWidget && (widgetRestricted.includes(widget.config.dataKey || '') || widgetRestricted.includes(widget.config.xAxis || ''));

                  // Data rows: cache first, then MongoDB dataset (with column restriction), else empty
                  const filteredData: any[] = isCacheWidget
                    ? (cacheEntry?.rows || [])
                  ...
                  return (
                    ...
                    >
                      ...
                      {(!isCacheWidget && !dataset) || isDatasetRestricted ? (
                        ...
                        <div className={`p-5 h-full flex flex-col ${isEditMode ? 'opacity-60 blur-[1px]' : ''}`}>
                          ...
                          <div className="flex-1 min-h-0 w-full">
                            <WidgetRenderer
                              widget={widget}
                              data={filteredData}
                              dataset={isCacheWidget ? { columns: cacheEntry?.columns || [] } : dataset}
                            />
                          </div>
                        </div>
                      )}
