...
export const generateQueryFromNaturalLanguage = async (
  ...
  try {
    ...
    try {
      const ai = getAI();
      if (!ai) return `// Error: AI Service unavailable — no API key`;
...
export const generateDashboardInsights = async (
  ...
  try {
    const ai = getAI();
    if (!ai) return "AI Insights unavailable — API key not configured.";
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
...
export const answerKnowledgeBaseQuestion = async (
  ...
  try {
    const ai = getAI();
    if (!ai) return "AI Assistant unavailable — API key not configured.";
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: [{ role: 'user', parts: [{ text: prompt }] }]
    });
