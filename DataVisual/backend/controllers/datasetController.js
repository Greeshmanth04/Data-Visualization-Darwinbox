...
export const updateDataset = async (req, res) => {
    try {
        ...
        if (req.user?.role === 'ADMIN') {
            editableFields.push('rowPolicies');
        }
        editableFields.forEach(f => {
            if (req.body[f] !== undefined) {
                dataset[f] = req.body[f];
            }
        });
...
export const connectSql = async (req, res) => {
    ...
    try {
        if (type === 'mysql') {
            const tables = await withMysql(uri, async (c) => {
...
export const saveExternalDataset = async (req, res) => {
    ...
    try {
        const encryptedConfig = encrypt(
            typeof connectionConfig === 'string' ? connectionConfig : JSON.stringify(connectionConfig)
        );
                ...
                Readable.from(buffer.toString('utf-8')).pipe(csv())
                    .on('data', (row) => {
                        Object.keys(row).forEach(k => {
                            if (!isNaN(row[k]) && !isNaN(parseFloat(row[k]))) {
...
export const queryDataset = async (req, res) => {
    ...
    try {
        ...
        if (sourceType === 'mysql') {
            ...
            rows = await withMysql(mysqlUri, async (c) => {
                const [r] = await c.execute(query);
                return r;
            });
        } else if (sourceType === 'postgres') {
            assertSelect(query);
            const pgConn = config.uri
                ? { connectionString: config.uri }
                : { host: config.host, port: config.port, user: config.user, password: config.password, database: config.database };
            rows = await withPostgres(
                pgConn.connectionString || `postgres://${config.user}:${config.password}@${config.host}:${config.port}/${config.database}`,
                async (c) => (await c.query(query)).rows
            );
...
export const saveSqlView = async (req, res) => {
    ...
    try {
        ...
        if (!source) {
            return res.status(404).json({ message: 'Source dataset not found' });
        }

        const base = {
            id: `ds_view_${Date.now()}`,
            name,
...
export const updateRowPolicies = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Failed to update row policies: ' + e.message });
    }
};
